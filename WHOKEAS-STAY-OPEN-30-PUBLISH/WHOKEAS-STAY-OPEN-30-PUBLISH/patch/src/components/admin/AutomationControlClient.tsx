                ...
                {[
                  ...
                  <label key={String(key)}>
                    ...
                    <input
                      ...
                      value={String(key) === "markupPercent" ? 30 : String(key) === "minimumProfitLocal" ? 0 : Number(value)}
                      readOnly={String(key) === "markupPercent" || String(key) === "minimumProfitLocal"}
                      disabled={String(key) === "markupPercent" || String(key) === "minimumProfitLocal"}
                      onChange={(event) => {
                        if (String(key) === "markupPercent" || String(key) === "minimumProfitLocal") return;
                        updateMarket(index, {
                          [key]: Number(event.target.value),
                        } as Partial<AutomationMarketRule>);
                      }}
                      className={`${fieldClass} ${
                        String(key) === "markupPercent" || String(key) === "minimumProfitLocal"
                          ? "cursor-not-allowed bg-[#f3efe7]"
                          : ""
                      }`}
                    />
                  </label>
      ...
      <section className="border border-[#d9d0c1] bg-[#fffdf9]">
        <div className="flex flex-col gap-3 border-b border-[#e4ddd2] bg-[#faf7f1] px-5 py-5 sm:flex-row sm:items-end sm:justify-between sm:px-6">
          <div>
            ...
            <h2 className="mt-1 font-serif text-3xl font-semibold">
              Categories, searches and fixed margin
            </h2>
          </div>
