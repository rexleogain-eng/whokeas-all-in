export const SITE_NAME = "WHOKEAS ALL IN";
export const SITE_URL = "https://www.whokeas.store";
export const PRODUCTS_URL = `${SITE_URL}/products`;
export const RETURN_POLICY_URL = `${SITE_URL}/returns-refunds`;
export const SHIPPING_POLICY_URL = `${SITE_URL}/shipping-delivery`;

export const US_TARGET_COUNTRY_CODE = "US";
export const US_SHIPPING_MIN_DAYS = 2;
export const US_SHIPPING_MAX_DAYS = 30;
export const US_RETURN_DAYS = 30;

export const SITE_DESCRIPTION =
  "Shop curated tech, home, study and everyday essentials with USD pricing, free standard U.S. shipping, guest checkout and direct WHOKEAS order support.";

export const BRAND_ALTERNATE_NAMES = [
  "WHOKEAS",
  "Whokeas Store",
] as const;

export const BRAND_LOGO_URL = `${SITE_URL}/brand/search-logo.png`;
export const FAVICON_URL = `${SITE_URL}/whokeas-favicon.png`;
export const DEFAULT_SOCIAL_IMAGE = BRAND_LOGO_URL;
