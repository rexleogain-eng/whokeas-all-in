...
const sampleEconomics = calculateGiftEconomics({
  sellingPriceUsd: 30,
  landedCostUsd: 20,
});
    ...
    <main className="min-h-screen bg-[#f4efe6] text-[#1d1914]">
      ...
      <section className="relative overflow-hidden border-b border-[#2d2923] bg-[#11100e] text-white">
        ...
        <div className="relative mx-auto grid max-w-[1500px] gap-12 px-6 py-16 sm:px-10 lg:grid-cols-[1.05fr_.95fr] lg:px-16 lg:py-24">
          ...
          <div className="relative min-h-[430px] border border-white/10 bg-[#1b1916] p-6 sm:p-9">
            <div className="absolute right-5 top-5 border border-[#d6bd7b]/50 px-3 py-1 text-[9px] font-black uppercase tracking-[0.16em] text-[#d6bd7b]">
              WHOKEAS branded
            </div>
            <div className="flex h-full min-h-[370px] flex-col items-center justify-center text-center">
              <div className="flex h-44 w-44 items-center justify-center rounded-full border border-[#d6bd7b]/25 bg-black/30 p-7 shadow-[0_30px_90px_rgba(0,0,0,.35)] sm:h-52 sm:w-52">
                <img
                  src="/brand/logo-mark.png"
                  alt="WHOKEAS logo mark"
                  className="h-full w-full object-contain"
                />
              </div>
              <p className="mt-8 text-[10px] font-black uppercase tracking-[0.26em] text-[#d6bd7b]">
                WHOKEAS ALL IN
              </p>
              <p className="mt-3 max-w-md font-serif text-3xl leading-tight text-white">
                Your branded piece, prepared after personalization approval.
              </p>
              <p className="mt-4 max-w-md text-xs leading-6 text-white/50">
                Final production method, landed cost, proof and delivery estimate are confirmed before payment is requested.
              </p>
            </div>
          </div>
        </div>
      ...
      <section id="collection" className="mx-auto max-w-[1500px] px-5 py-14 sm:px-8 lg:py-20">
        <div className="flex flex-wrap items-end justify-between gap-5">
          <div>
            ...
            <h2 className="mt-3 max-w-4xl font-serif text-4xl font-semibold sm:text-5xl">
              Branded products customers can order individually.
            </h2>
          </div>
          <p className="max-w-xl text-sm leading-7 text-[#6b6358]">
            No placeholder selling prices are published. Each custom order is quoted from the verified production, personalization and U.S. fulfilment cost before the secure payment request is sent.
          </p>
        </div>
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {brandedProducts.map((product) => (
            <article
              key={product.name}
              className="group flex min-h-full flex-col border border-[#d7cebf] bg-[#fffdf8] transition hover:-translate-y-1 hover:border-[#b89a59] hover:shadow-[0_22px_55px_rgba(38,30,20,0.10)]"
            >
              <div className="relative flex aspect-[4/3] items-center justify-center overflow-hidden border-b border-[#e1d8ca] bg-[#171512] p-7">
                <div className="absolute left-4 top-4 text-[9px] font-black uppercase tracking-[0.16em] text-[#d6bd7b]">
                  {product.type}
                </div>
                <div className="absolute right-4 top-4 border border-white/15 px-2 py-1 text-[8px] font-black uppercase tracking-[0.14em] text-white/55">
                  {product.mark}
                </div>
                <div className="flex h-28 w-28 items-center justify-center rounded-full border border-[#d6bd7b]/30 bg-black/30 p-5 transition duration-500 group-hover:scale-105">
                  <img
                    src="/brand/logo-mark.png"
                    alt="WHOKEAS logo"
                    className="h-full w-full object-contain"
                ...
                <div className="absolute bottom-4 left-4 right-4 text-center text-[9px] font-black uppercase tracking-[0.2em] text-[#d6bd7b]">
                  WHOKEAS ALL IN
                </div>
              </div>
              ...
              <div className="flex flex-1 flex-col p-5">
                <div className="flex flex-wrap gap-2">
                  <span className="border border-[#d7cebf] bg-[#f7f2e9] px-2 py-1 text-[8px] font-black uppercase tracking-[0.12em] text-[#6b6358]">
                    Sold separately
                  </span>
                  {product.gift && (
                    <span className="border border-[#c9b98f] bg-[#f4ead0] px-2 py-1 text-[8px] font-black uppercase tracking-[0.12em] text-[#73571d]">
                      Gift eligible
                    </span>
                  )}
                </div>
                ...
                <div className="mt-5 border-t border-[#e1d8ca] pt-4">
                  <p className="text-[9px] font-black uppercase tracking-[0.14em] text-[#9b762c]">
                    Personalize
                  </p>
      ...
      <section className="border-y border-[#d7cebf] bg-[#171512] text-white">
        <div className="mx-auto grid max-w-[1500px] gap-10 px-5 py-14 sm:px-8 lg:grid-cols-[.9fr_1.1fr] lg:py-20">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.24em] text-[#d6bd7b]">
              WHOKEAS gift economics
            </p>
            <h2 className="mt-4 max-w-2xl font-serif text-4xl font-semibold sm:text-5xl">
              A gift can delight the customer without consuming the sale.
            </h2>
          ...
          <div className="border border-white/12 bg-white/[0.04] p-6 sm:p-8">
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#d6bd7b]">
              Example
            </p>
            <div className="mt-5 grid grid-cols-2 gap-px bg-white/10 sm:grid-cols-4">
              {[
                ["Sale", `$${sampleEconomics.sellingPriceUsd.toFixed(2)}`],
                ["Landed cost", `$${sampleEconomics.landedCostUsd.toFixed(2)}`],
                ["Gross profit", `$${sampleEconomics.grossProfitUsd.toFixed(2)}`],
                ["Max gift cost", `$${sampleEconomics.maxGiftCostUsd.toFixed(2)}`],
              ].map(([label, value]) => (
            ...
            <div className="mt-5 border border-[#d6bd7b]/25 bg-[#d6bd7b]/[0.06] p-5">
              <p className="text-xs leading-6 text-white/70">
                A $30.00 sale with $20.00 landed cost produces $10.00 gross profit. The promotional gift may cost at most $1.50, leaving <strong className="text-white">${sampleEconomics.profitAfterGiftUsd.toFixed(2)}</strong> of gross profit before other applicable fees.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="border-b border-[#d7cebf] bg-[#ece2d2]">
        <div className="mx-auto grid max-w-[1500px] gap-0 lg:grid-cols-3">
          {[
            ["01", "Every gift is also a product", "Gift-eligible mugs, tumblers, totes, keychains, notebooks and bottles remain available for customers to order separately."],
            ["02", "15% profit ceiling", "A promotional gift is approved only when its real cost fits within 15% of the gross profit generated by that qualifying order."],
            ["03", "Verified before payment", "Production method, personalization, landed cost and U.S. delivery are confirmed before WHOKEAS sends the secure payment request."],
      ...
      <footer className="bg-[#12110f] px-5 py-10 text-center text-sm text-white/55">
        © 2026 WHOKEAS ALL IN · Branded & Personalized Collection
      </footer>
    </main>
  );
}
