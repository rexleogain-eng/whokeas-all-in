...
$homePath = (Resolve-Path ".\src\app\page.tsx").Path
$home = [System.IO.File]::ReadAllText($homePath)

if ($home -notmatch 'components/store/CartButton') {
  $home = $home.Replace(
    'import Link from "next/link";',
...
$homeUpdated = [regex]::Replace($home, $staticCartPattern, '<CartButton />', 1)

if ($homeUpdated -eq $home) {
  Write-Warning "The homepage static cart button was not found. Product and cart pages were still created."
}

[System.IO.File]::WriteAllText($homePath, $homeUpdated, $utf8)
